...
function HomeSearch() {
    ...
    const [startAddressSearchModal, setStartAddressSearchModal] =
        useState(false);
    ...
    const getProvinces = () => {
        axios.get("https://provinces.open-api.vn/api/").then((res) => {
            setProvinces(
                res.data.reduce((previousValue, currentValue) => {
                    currentValue.name = currentValue.name.replace("Tỉnh", "");
                    currentValue.name = currentValue.name.replace(
                        "Thành phố",
                        ""
                    );
                    currentValue.name = currentValue.name.trim();
                                            ...
                                            : provinces
                                                  .filter((item) =>
                                                      item
                                                          .toLowerCase()
                                                          .includes(
                                                              startAddressSearch.toLowerCase()
                                                          )
                                                  )
                                                  .map((item, i) => (
                                                      <li
                            ...
                            {endAddressSearchModal && (
                                <div className="absolute top-0 left-0 w-full bg-white rounded-xl shadow-lg ">
                                    <input
                                        ...
                                        type="text"
                                        onChange={(e) =>
                                            setEndAddressSearch(e.target.value)
                                        }
                                    />
                                    <ul className="w-full overflow-y-scroll max-h-52">
                                        {endAddressSearch === ""
